...
from .custom import conv3d, conv2d, batch_norm, concat, LReluLayer, ReluLayer, SubVoxelConv,  max_pool3d, max_pool2d

__all__ = ['unet3d', 'unet2d']
'''
unet for 3d
'''
...
def unet2d(LR, ngf=32, reuse=False, name='unet2d'):
    '''
    Params:
        LR - [batch, depth, height, width, channels]
    '''
    ...
    with tf.variable_scope(name, reuse=reuse):
        n = tl.layers.InputLayer(LR, name='lr_input')
        n = conv2d(n, ngf, f_size, 1, act=act, name='conv0')
        layers.append(n)
        ...
        for out_channels in layer_specs:
            with tf.variable_scope('encoder_%d' % (len(layers) + 1)):
                # rect = LReluLayer(layers[-1], alpha=0.2)
                conv = conv2d(layers[-1], out_channels, f_size, 1, act=act, name='down_conv1')
                conv = conv2d(conv, out_channels, f_size, 1, act=act, name='down_conv2')
                pool = max_pool2d(conv, filter_size=f_size, stride=2, padding='SAME', name='maxpool')
                layers.append(pool)
                print(pool.outputs.shape)
        ...
        with tf.variable_scope('out'):
            n = concat([layers[-1], layers[0]])
            output = conv2d(n, 1, f_size, 1, act=tf.identity, name='conv')
            return output