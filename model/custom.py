...
from tensorlayer.layers import Layer, Conv3dLayer, DeConv3dLayer, Conv2dLayer, DeConv2dLayer, ConcatLayer, \
    BatchNormLayer,  MaxPool3d, MaxPool2d
...
def conv3d_transpose(input, out_channels, filter_size, stride, act=None, padding='SAME', name='conv3d_transpose'):
    ...
    return DeConv3dLayer(input, act=act, shape=shape, output_shape=output_shape, strides=strides, padding=padding,
...
def conv3d_transpose2(input, out_channels, filter_size, stride, padding='SAME', name='conv3d_transpose'):
    return tf.layers.Conv3DTranspose(filters=out_channels, kernel_size=(filter_size, filter_size, filter_size),
                                     strides=(stride, stride, stride), padding=padding, kernel_initializer=w_init,
                                     bias_initializer=b_init, name=name);


def conv3d(layer, out_channels, filter_size=3, stride=1, act=tf.identity, padding='SAME', W_init=w_init, b_init=b_init,
           name='conv3d'):
    """
    Params
    ...
    strides = [1, stride, stride, stride, 1]
    return Conv3dLayer(layer, act=act, shape=shape, strides=strides, padding=padding, W_init=W_init, b_init=b_init,
                       name=name)


def conv2d(layer, out_channels, filter_size=3, stride=1, act=tf.identity, padding='SAME', W_init=w_init, b_init=b_init,
           name='conv2d'):
    in_channels = layer.outputs.get_shape().as_list()[-1]
...
def conv3d2(input, out_channels, filter_size=3, stride=1, padding='SAME', name='conv3d'):
    ...
    with tf.variable_scope(name):
        weight = tf.get_variable(name='weight_conv3d', shape=filter_shape,
...
def max_pool2d(x, filter_size=3, stride=2, padding='SAME', name='maxpool2d'):
    filter_size = [filter_size, filter_size, filter_size]
    strides = [stride, stride]
    return MaxPool2d(x, filter_size=filter_size, strides=strides, padding=padding, name=name)
...
def _interpolateXY(data, ratio, nchannels_out=1):
    ...
    Parameters
    3d data [batch_size, depth, height, width, nchannels]
    2d data [batch_size, height, width, nchannels]
    """
    assert len(data.shape) == 5 or len(data.shape) == 4, "incompatible data dimension"
    if len(data.shape) == 5:
        ...
        bsize, h, w, c = data.get_shape().as_list()
        assert c == (ratio ** 2) * nchannels_out, "nchannels_out: %d , nchannels_in: %d, ratio: %d\n" % (
            nchannels_out, c, ratio)

        Xs_ = tf.split(data, ratio, -1)  # split channels
        Xr_ = tf.concat(Xs_, 2)  # concat at width dimension
        # print(bsize, d, h*ratio, w*ratio, nchannels_out)
        X_ = tf.reshape(Xr_, (bsize,  h * ratio, w * ratio, int(nchannels_out)))
    return X_


def SubVoxelConv(net, scale=2, only_z=False, n_out_channel=None, act=tf.identity, name='subpixel_conv3d'):
    """
    Inflate pixles in [channels] into [depth height width] to interpola the 3-d image  or Inflate pixles in [channels] into [height width] to interpola the 2-d image
    Parameters
    net for 3d : [batch depth height width channels]
    net for 2d : [batch height width channels]
    ...
    def _PS(X, r, n_out_channel, z_only=False):
        if n_out_channel >= 1:
            ...
            if len(X.get_shape()) == 4:
                ...
                batch_size = tf.shape(X)[0]
                Xs = _interpolateXY(X, scale, nchannels_out=int(X.get_shape()[-1]) / (scale ** 2))
                X = tf.reshape(Xs, (batch_size, height * scale, width * scale, n_out_channel))
        return X

    inputs = net.outputs

    down_factor = scale ** 3 if len(inputs.get_shape()) == 5 else scale ** 2
    if only_z:
        down_factor = scale
