...
from utils import normalize_percentile, normalize_min_max, _raise, get_file_list, imread2d, imwrite2d
...
class Model:
    ...
    def predict(self, input):
        len(input.shape) == 3 or len(input.shape) == 2 or _raise(ValueError(''))
...
class Predictor:
    def __init__(self, factor, block_size, model, dtype=np.float32):
        self.factor = factor
    ...
    def __check_dims(self, im, block_size, overlap):
        ...
        len(im.shape) == 3 or len(im.shape) == 2 or _raise(ValueError('the input image must be in shape [depth, height, width]'))
