...
from model import RDN3d, DBPN3d, unet3d, unet2d, Conv_net3d, drunet3d, drunet2d

from model.losses import l2_loss, edges_loss, img_gradient_loss, l1_loss, lpips_loss
...
using_lpips_loss = config.TRAIN.using_lpips_loss
...
class BaseTrainer:
    ...
    def __init__(self, dataset, archi1='dbpn3d', archi2='rdn3d', visualize_features=False, pretrain=False):
        archi1 in [None, 'dbpn3d', 'convnet3d'] or _raise(ValueError('illegal argument archi1: %s' % archi1))
        archi2 in ['rdn3d', 'drunet3d', 'unet3d', 'unet2d', 'drunet2d'] or _raise(ValueError('illegal argument archi2: %s' % archi2))
    ...
    def _load_designated_ckpt(self, begin_epoch, sess):
        ...
        if (begin_epoch != 0):
            if (self.archi1 is not None):
                ...
                load_and_assign_ckpt(sess, checkpoint_dir + '/{}'.format(ckpt), self.net) or _raise(
                    RuntimeError('load and assigened ckpt failed'))
    ...
    def _get_output_node_name(self, sess):
        ...
        for op in sess.graph.get_operations():
            if 'out' in op.name:
                name = op.name
        name or _raise(ValueError('ops with name "out" is not in the default graph'))
        print('\noutput node found : %s\n' % name)
        return name
    ...
    def _save_half_precision_pb(self):
        ...
        output_names = ["net_s2/out/Tanh"]  # "net_s2/out/Identity"
        convert_graph_to_fp16(model_path, save_path, name, as_text=as_text, target_type=target_type,
        ...
        for idx in range(0, len(self.test_lr), batch_size):
            if idx + batch_size <= len(self.test_lr):
                ...
                if (self.archi1 is not None):
                    ...
                    feed_test = {self.plchdr_lr: test_lr_batch, self.plchdr_hr: test_hr_batch,
                                 self.plchdr_mr: test_mr_batch}
                    out_resolver, out_interp, summary_t_loss = sess.run(
                        [self.resolver.outputs, self.interpolator.outputs, self.summary_op_test], feed_test)
    ...
    def _train(self, begin_epoch=0, test=True, verbose=True):
        ...
        while training_dataset.hasNext():
            ...
            if (self.archi1 is not None):
                evaluated = sess.run(fetches,
                                     {self.plchdr_lr: LR_batch, self.plchdr_hr: HR_batch, self.plchdr_mr: MR_batch})
            else:
                evaluated = sess.run(fetches, {self.plchdr_lr: LR_batch, self.plchdr_hr: HR_batch})
            print("\rEpoch:[%d/%d] iter:[%d/%d] times: %4.3fs     " % (
            epoch, n_epoch, cursor + 1, n_training_pairs, time.time() - step_time), end='')
...
class Trainer(BaseTrainer):
    ...
    def build_graph(self):
        ...
        with tf.device('/gpu:1'):
            ...
            if self.archi == 'rdn3d':
                net = RDN3d(self.plchdr_lr, factor=config.factor, reuse=False, bn=using_batch_norm,
                                    name=variable_tag)
                net_test = RDN3d(self.plchdr_lr, factor=config.factor, reuse=True, bn=using_batch_norm,
                                         name=variable_tag)

            elif self.archi == 'unet3d':
            ...
            elif self.archi == 'dbpn3d':
                net = DBPN3d(self.plchdr_lr, upscale=True, factor=config.factor, reuse=False, name=variable_tag)
                net_test = DBPN3d(self.plchdr_lr, upscale=True, factor=config.factor, reuse=True, name=variable_tag)
            elif self.archi == 'drunet3d':
                net = drunet3d(self.plchdr_lr,  reuse=False, name=variable_tag)
                net_test = drunet3d(self.plchdr_lr,  reuse=True, name=variable_tag)
            elif self.archi == 'unet2d':
                self.plchdr_lr = tf.placeholder("float", [batch_size] + hr_size, name="LR")
                net = unet2d(self.plchdr_lr, reuse=False, name=variable_tag)
                net_test = unet2d(self.plchdr_lr, reuse=True, name=variable_tag)
            elif self.archi == 'drunet2d':
                self.plchdr_lr = tf.placeholder("float", [batch_size] + hr_size, name="LR")
                net = drunet2d(self.plchdr_lr, reuse=False, name=variable_tag)
                net_test = drunet2d(self.plchdr_lr, reuse=True, name=variable_tag)
            else:
        ...
        if using_lpips_loss:
            ...
            l_optim = tf.train.AdamOptimizer(self.learning_rate_var, beta1=beta1).minimize(loss_lpips,
                                                                                           var_list=net_vars)
            self.loss.update({'lpips_loss': loss_lpips})
            self.optim.update({'l_optim': l_optim})

        if using_edge_loss:
            loss_edges = edges_loss(net.outputs, self.plchdr_hr)
            e_optim = tf.train.AdamOptimizer(self.learning_rate_var, beta1=beta1).minimize(loss_edges,
                                                                                           var_list=net_vars)
...
class SegmentedTrainer(BaseTrainer):
    ...
    def build_graph(self):
        ...
        with tf.device('/gpu:%d' % 1):
            if self.archi1 == 'dbpn3d':
                net_stage1 = DBPN3d(self.plchdr_lr, upscale=False, name=variable_tag_res)
                net_stage1_test = DBPN3d(self.plchdr_lr, upscale=False, reuse=True, name=variable_tag_res)
            elif self.archi1 == 'convnet3d':
                net_stage1 = Conv_net3d(self.plchdr_lr, reuse=False, name=variable_tag_res)
                net_stage1_test = Conv_net3d(self.plchdr_lr, reuse=True, name=variable_tag_res)
            else:
                _raise(ValueError())

        with tf.device('/gpu:%d' % 2):
            if self.archi2 == 'rdn3d':
                net_stage2 = RDN3d(net_stage1.outputs, factor=config.factor, conv_kernel=conv_kernel,
                                           bn=using_batch_norm, is_train=True, name=variable_tag_interp)
                net_stage2_test = RDN3d(net_stage1_test.outputs, factor=config.factor, conv_kernel=conv_kernel,
                                                bn=using_batch_norm, reuse=True, is_train=False,
                                                name=variable_tag_interp)
            else:
                _raise(ValueError())
        ...
        if using_lpips_loss:
            loss_lpips = lpips_loss(net_stage2.outputs, self.plchdr_hr)
            l_optim = tf.train.AdamOptimizer(self.learning_rate_var, beta1=beta1).minimize(loss_lpips, var_list=vars_n2)
            self.loss.update({'lpips_loss': loss_lpips})
            self.optim.update({'l_optim': l_optim})
